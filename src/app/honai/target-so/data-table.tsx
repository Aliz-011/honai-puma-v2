                                    ...
                                    {!isHeaderRow && (
                                        <>
                                            <TableCell className="px-1 py-0.5 border-r last:border-r-0 text-end dark:text-white dark:border-gray-800 !tabular-nums">
                                                <span className='text-end'>{formatToBillion(item.target_so)}</span>
                                            </TableCell>
                                            <TableCell className="px-1 py-0.5 border-r last:border-r-0 text-end dark:text-white dark:border-gray-800 !tabular-nums">
                                                <span className='text-end'>{formatToBillion(item.so)}</span>
                                            </TableCell>
                                            <TableCell className={cn("px-1 py-0.5 border-r last:border-r-0 text-end dark:text-white dark:border-gray-800 !tabular-nums", item.gap_to_target_so > 0 ? 'text-green-500' : 'text-rose-500')}>
                                                <span>{formatToBillion(item.gap_to_target_so)}</span>
                                            </TableCell>
                                            <TableCell className={cn("px-1 py-0.5 border-r last:border-r-0 text-end text-theme-xs dark:text-white dark:border-gray-800 !tabular-nums", getAchGrowthColor(item.ach_target_fm_so) ? 'text-green-500' : 'text-rose-500')}>
                                                {item.ach_target_fm_so}
                                            </TableCell>
                                            <TableCell className={cn("px-1 py-0.5 border-r last:border-r-0 text-end text-theme-xs dark:text-white dark:border-gray-800 !tabular-nums", getAchGrowthColor(item.drr_so) ? 'text-green-500' : 'text-rose-500')}>
                                                {item.drr_so}
                                            </TableCell>
                                            <TableCell className={cn("px-1 py-0.5 border-r last:border-r-0 text-end text-theme-xs font-medium dark:border-gray-800 tabular-nums", getGrowthColor(item.mom_so) ? 'text-green-500' : 'text-rose-500')}>
                                                {item.mom_so}
                                            </TableCell>
...
type Revenue = {
    ...
    target_so: number;
    so: number;
    ach_target_fm_so: string;
    drr_so: string;
    gap_to_target_so: number;
    mom_so: string;
    so_absolut: number;
    yoy_so: string;
    ytd_so: string;
