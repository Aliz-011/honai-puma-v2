...
const Page = () => {
    ...
    const { data, isLoading, isError, error, refetch, isSuccess, isRefetching } = useQuery({
        queryKey: ['consolidation-all', selectedDate, branch, cluster, kabupaten],
        queryFn: async ({ queryKey }) => {
            ...
            const response = await client.api['consolidation-all'].$get({
                query: {
                    date: dateString,
        ...
        <div className="overflow-hidden min-h-screen rounded-2xl border border-gray-200 px-5 py-7 dark:border-gray-800 dark:bg-white/[0.03] space-y-4 bg-gray-50">
            ...
            {(() => {
                ...
                if (isDataActuallyAvailable) {
                    return (
                        <>
                            <DataTable refetch={refetch} data={data} latestUpdatedData={2} title="REV MOBILE ALL vs REV HH ALL" date={selectedDate} isLoading={isLoading || isRefetching} />
                        </>
                    )
                }
