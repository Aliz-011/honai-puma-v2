...
export default function RevenueSAPage() {
    ...
    const { data, isLoading, isRefetching, refetch } = useQuery({
        ...
        queryFn: async () => {
            const response = await client.api['revenue-sa-prabayar'].$get({ query: { date: date?.toLocaleString() } })
    ...
    return <div className="px-4 lg:px-6">
        <div className="overflow-hidden min-h-screen rounded-2xl border border-gray-200 bg-white px-5 py-7 dark:border-gray-800 dark:bg-white/[0.03] space-y-4">
            ...
            <DataTable refetch={refetch} data={data} latestUpdatedData={2} title="Revenue SA Prabayar" date={date} isLoading={isLoading || isRefetching} />
        </div>
    </div>
}
