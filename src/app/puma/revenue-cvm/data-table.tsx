...
export function DataTable({ latestUpdatedData: daysBehind, refetch, title, data, isLoading, date }: Params) {
    ...
    const formattedData = data.map(item => ({
        territory: item.name,
        target: item.target_rev_cvm,
        rev: item.rev_cvm,
        gap: item.gap_to_target_cvm,
        achFm: item.ach_target_fm_cvm,
        drr: item.drr_cvm,
        mom: item.mom_cvm,
        abs: item.rev_cvm_absolut,
        yoy: item.yoy_cvm,
        ytd: item.ytd_cvm,
        qoq: item.qoq_cvm
    }))
    ...
    const handleDownload = async () => {
        exportToExcel(formattedData, format(selectedDate, 'd MMM'), `revenue_cvm_${format(selectedDate, 'yyyyMM')}`)
    }
                                    ...
                                    {!isHeaderRow && (
                                        <>
                                            <TableCell className="px-1 py-0.5 border-r last:border-r-0 text-end dark:text-white dark:border-gray-800 !tabular-nums">
                                                <span className='text-end'>{item.target_rev_cvm}</span>
                                            </TableCell>
                                            <TableCell className="px-1 py-0.5 border-r last:border-r-0 text-end dark:text-white dark:border-gray-800 !tabular-nums">
                                                <span className='text-end'>{item.rev_cvm}</span>
                                            </TableCell>
                                            <TableCell className={cn("px-1 py-0.5 border-r last:border-r-0 text-end dark:text-white dark:border-gray-800 !tabular-nums", item.gap_to_target_cvm > 0 ? 'text-green-500' : 'text-rose-500')}>
                                                <span>{item.gap_to_target_cvm}</span>
                                            </TableCell>
                                            <TableCell className={cn("px-1 py-0.5 border-r last:border-r-0 text-end text-theme-xs dark:text-white dark:border-gray-800 !tabular-nums", getAchGrowthColor(item.ach_target_fm_cvm) ? 'text-green-500' : 'text-rose-500')}>
                                                {item.ach_target_fm_cvm}
                                            </TableCell>
                                            <TableCell className={cn("px-1 py-0.5 border-r last:border-r-0 text-end text-theme-xs dark:text-white dark:border-gray-800 !tabular-nums", getAchGrowthColor(item.drr_cvm) ? 'text-green-500' : 'text-rose-500')}>
                                                {item.drr_cvm}
                                            </TableCell>
                                            <TableCell className={cn("px-1 py-0.5 border-r last:border-r-0 text-end text-theme-xs font-medium dark:border-gray-800 tabular-nums", getGrowthColor(item.mom_cvm) ? 'text-green-500' : 'text-rose-500')}>
                                                {item.mom_cvm}
                                            </TableCell>
                                            <TableCell className={cn("px-1 py-0.5 border-r last:border-r-0 text-end dark:text-white dark:border-gray-800 !tabular-nums", item.rev_cvm_absolut > 0 ? 'text-green-500' : 'text-rose-500')}>
                                                <span className='text-end'>{item.rev_cvm_absolut}</span>
                                            </TableCell>
                                            <TableCell className={cn("px-1 py-0.5 border-r last:border-r-0 text-end text-theme-xs dark:text-white dark:border-gray-800 !tabular-nums", getGrowthColor(item.yoy_cvm) ? 'text-green-500' : 'text-rose-500')}>
                                                {item.yoy_cvm}
                                            </TableCell>
                                            <TableCell className={cn("px-1 py-0.5 border-r last:border-r-0 text-end text-theme-xs dark:text-white dark:border-gray-800 !tabular-nums", getGrowthColor(item.ytd_cvm) ? 'text-green-500' : 'text-rose-500')}>
                                                {item.ytd_cvm}
                                            </TableCell>
                                            <TableCell className={cn("px-1 py-0.5 border-r last:border-r-0 text-end text-theme-xs font-medium dark:border-gray-800 !tabular-nums", getGrowthColor(item.qoq_cvm) ? 'text-green-500' : 'text-rose-500')}>
                                                {item.qoq_cvm}
                                            </TableCell>
                                        </>
                                    )}
...
type Revenue = {
    name: string | null;
    target_rev_cvm: number;
    rev_cvm: number;
    ach_target_fm_cvm: string;
    drr_cvm: string;
    gap_to_target_cvm: number;
    mom_cvm: string;
    rev_cvm_absolut: number;
    yoy_cvm: string;
    ytd_cvm: string;
    qoq_cvm: string;
}