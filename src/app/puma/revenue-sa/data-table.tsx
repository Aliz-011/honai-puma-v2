                                    ...
                                    {!isHeaderRow && (
                                        <>
                                            <TableCell className="px-1 py-0.5 border-r last:border-r-0 text-end dark:text-white dark:border-gray-800 !tabular-nums">
                                                <span className='text-end'>{formatToBillion(item.target_rev_sa)}</span>
                                            </TableCell>
                                            <TableCell className="px-1 py-0.5 border-r last:border-r-0 text-end dark:text-white dark:border-gray-800 !tabular-nums">
                                                <span className='text-end'>{formatToBillion(item.rev_sa)}</span>
                                            </TableCell>
                                            <TableCell className={cn("px-1 py-0.5 border-r last:border-r-0 text-end dark:text-white dark:border-gray-800 !tabular-nums", item.gap_to_target_sa > 0 ? 'text-green-500' : 'text-rose-500')}>
                                                <span>{formatToBillion(item.gap_to_target_sa)}</span>
                                            </TableCell>
                                            <TableCell className={cn("px-1 py-0.5 border-r last:border-r-0 text-end text-theme-xs dark:text-white dark:border-gray-800 !tabular-nums", getAchGrowthColor(item.ach_target_fm_sa) ? 'text-green-500' : 'text-rose-500')}>
                                                {item.ach_target_fm_sa}
                                            </TableCell>
                                            <TableCell className={cn("px-1 py-0.5 border-r last:border-r-0 text-end text-theme-xs dark:text-white dark:border-gray-800 !tabular-nums", getAchGrowthColor(item.drr_sa) ? 'text-green-500' : 'text-rose-500')}>
                                                {item.drr_sa}
                                            </TableCell>
                                            <TableCell className={cn("px-1 py-0.5 border-r last:border-r-0 text-end text-theme-xs font-medium dark:border-gray-800 tabular-nums", getGrowthColor(item.mom_sa) ? 'text-green-500' : 'text-rose-500')}>
                                                {item.mom_sa}
                                            </TableCell>
                                            <TableCell className={cn("px-1 py-0.5 border-r last:border-r-0 text-end dark:text-white dark:border-gray-800 !tabular-nums", item.rev_sa_absolut > 0 ? 'text-green-500' : 'text-rose-500')}>
                                                <span className='text-end'>{formatToBillion(item.rev_sa_absolut)}</span>
                                            </TableCell>
                                            <TableCell className={cn("px-1 py-0.5 border-r last:border-r-0 text-end text-theme-xs dark:text-white dark:border-gray-800 !tabular-nums", getGrowthColor(item.yoy_sa) ? 'text-green-500' : 'text-rose-500')}>
                                                {item.yoy_sa}
                                            </TableCell>
                                            <TableCell className={cn("px-1 py-0.5 border-r last:border-r-0 text-end text-theme-xs dark:text-white dark:border-gray-800 !tabular-nums", getGrowthColor(item.ytd_sa) ? 'text-green-500' : 'text-rose-500')}>
                                                {item.ytd_sa}
                                            </TableCell>
                                        </>
                                    )}
...
type Revenue = {
    name: string | null;
    target_rev_sa: number;
    rev_sa: number;
    ach_target_fm_sa: string;
    drr_sa: string;
    gap_to_target_sa: number;
    mom_sa: string;
    rev_sa_absolut: number;
    yoy_sa: string;
    ytd_sa: string;
}