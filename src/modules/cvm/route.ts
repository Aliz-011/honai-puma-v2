...
import { dynamicRevenueCVM, dynamicRevenueCVMOutlet, summaryBbRegional, summaryBbBranch, summaryBbSubbranch, summaryBbCluster, summaryBbCity, feiTargetPuma } from "@/db/schema/v_honai_puma";
import { territoryArea4 } from '@/db/schema/puma_2025'
import { db } from "@/db";
import { Regional } from "@/types";

const app = new Hono()
    .get('revenue-cvm', zValidator('query', z.object({ date: z.coerce.date().optional() })),
        async c => {
    ...
    .get('revenue-cvm-outlet', zValidator('query', z.object({ date: z.coerce.date().optional() })),
        async c => {
            ...
            const revenueCVMOutlet = dynamicRevenueCVMOutlet(currYear, currMonth)

            const revenue = db
                .select()
                .from(revenueCVMOutlet)
                .where(eq(revenueCVMOutlet.processingDate, trxDate))
                .prepare()
    ...
    .get('/revenue-cvm-v2', zValidator('query', z.object({ date: z.coerce.date().optional() })),
        async c => {
            const { date } = c.req.valid('query')
            const selectedDate = date ? new Date(date) : subDays(new Date(), 2)
    ...
    .get('/revenue-cvm-outlet-v2', zValidator('query', z.object({ date: z.coerce.date().optional() })),
        async c => {
            const { date } = c.req.valid('query')
            const selectedDate = date ? new Date(date) : subDays(new Date(), 2)
